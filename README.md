...
### css
1. css 란?
    - Css 는 cascading style sheets의 약자로 스타일링을 위한 언어임
    - Cascading이란 계단식이라는 뜻으로 한 엘리먼에 여러 스타일이 적용될 경우 스타일간의 충돌을 막기 위해 계단식으로 스타일을 적용시키는 규칙을 갖고있음
    - 즉 하나의 스타일이 여러개의 엘리먼트에 적용될 수도 있고 하나의 엘리먼트에도여러개의 스타일이 적용될 수 있음
    - 엘리먼트에 스타일이 적용되는 규칙을 selector라고 함 css 는 이 선택자와 스타일로 이루어짐
    - 이번 장에서는 선택자와 스타일을 카테고리별로 나누어 학습함
2. css문법과 선택자
    - 선택자를 먼저 쓰고 다음에 적용할 스타일을 중괄호 안에 세미콜론으로 구분하여 하나씩 작성함
    - 선택자는 HTML 엘리먼트를 직접 넣어도 되고 엘리먼트의 조합 혹은 class의 형태로 작성 가능함
    - 스타일은 property와 key value 로 이루어 지며, 이들은 콜론으로 구분하고 각 스타일은 세미콜론으로 구분함
3. 레이아웃과 관련된 속성
    - 화면에 엘리먼트를 어떻게 배치할 것인지를 정의 함 
    - 가장 중요한 속성은 display
    - 모든 엘리먼트는 기본 display 속성을 갖고 있지만 이 기본값을 변경 해 줄 수 있음
    - none는 존재는 하지만 화면에 보이지 않는 것으로 자바스크립트를 넣을 때 많이 사용함
    - block 은 세로로 정렬되며 with의 heigh를 갖을 수 있다. 크기와 상관없이 한줄을 점유함
    - inline은 가로로 정렬되며 with  heigh를 갖을 수 없다. 크기만큼
    공간을 점유함    
    - inline-block은 기본적으로 inline의 특성을 갖지만 with와 height등 block의 특성을 사용 할 수 있음
    - visibility 속성은 엘리먼트의 가시성을 정의함
    - 여기서 중요한 것은 display:none 과 visibility:hidden의 차이임
    - display:none 은 엘리먼트의 영역이 보이지 않고 visibility:hidden 는 차지하는 영역은 보임
    - 다음으로 position 속성은 엘리먼트를 어떻게 위치시킬 것 인지 참고 함

### styled-components
- Styled Components를 사용하기 위해서는 가장 먼저, 아까 설치한 styled-components 패키지에서 styled를 import 해야 한다.
- 프로젝트의 규모가 커질 수록 각각의 파일의 관리가 어려워지기 때문에 하나의 파일 안에서 JS와 CSS를 모두 관리한다는 것은 큰 장점.
- CSS 파일에서 작성한 class와 id를 올바르게 작성했는지 파일을 이동하지 않아도 되고, Styled Components에서는 같은 컴포넌트 이름을 사용하더라도 다른 파일에 위치하고 있다면 랜덤한 className을 지정하여 주기 때문에 네이밍에 스트레스를 받지 않아도 되기 때문.

3. styled-components의 props 사용하기
    - props 를 이요하여 조건이나 동적으로 변하는 값을 사용해서 스타일링 할 수 있음
